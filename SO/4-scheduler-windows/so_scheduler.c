...
tid_t so_fork(so_handler *handler, unsigned priority)
{
	...
	threads[count].h = CreateThread(
		...
		0,
		&threads[count].tid);
	if (threads[count].h == INVALID_TID)
		return INVALID_TID;
	...
	if (count == 1) {
		...
		heap_pop(heap);
		rc = ReleaseSemaphore(threads[0].sem, 1, NULL);
...
void so_end(void)
{
	...
	for (i = 0 ; i < count ; i++) {
		rc = WaitForSingleObject(threads[i].h, INFINITE);

		if (rc == WAIT_FAILED)
			exit(1);

		rc = CloseHandle(threads[i].h);

		if (rc == 0)
			exit(1);

		rc = CloseHandle(threads[i].sem);
...
static DWORD WINAPI start_thread(LPVOID arg)
{
	...
	rc = WaitForSingleObject(threads[id].sem, INFINITE);
...
static void change_context(unsigned tid, char finish)
{
	...
	rc = ReleaseSemaphore(threads[current_thread].sem, 1, NULL);

	if (rc == 0)
		exit(1);

	if (finish == NOFINISH) {
		rc = WaitForSingleObject(threads[old].sem, INFINITE);
